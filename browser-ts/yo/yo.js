...
function writeFile(data) {
  try {
    const fileContents = JSON.stringify(data.contents);
    // throw { 'foo' : 'dummy-error'};
    fs.writeFileSync(data.filePath, fileContents);
    sendCommandToAppWindow('generator:file-written', true);
  } catch (err) {
    sendCommandToAppWindow('generator:error', {
...
function testEvent() {
  sendCommandToAppWindow('generator:test', {
    "foo": "bar"
  });
}
...
var api = {
  ...
  'generator:write-file': writeFile,
